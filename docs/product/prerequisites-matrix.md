...
| Ansible collections | Local tooling | all | always | mandatory | Required because database and monitoring playbooks reference modules from `community.mysql` | `ansible-doc -t module community.mysql.mysql_db` | Yes (`ansible-galaxy`) | Yes |
...
| Apache TLS component | Web/TLS | app | `WEB_SERVER_TYPE=apache` + `TLS_MODE!=none` | conditional-mandatory | Apache must load SSL and own the configured HTTPS listener | Debian: `apachectl -M`; Rocky: `mod_ssl` + `httpd -M` | Yes through automatic package/module resolution | Yes |
